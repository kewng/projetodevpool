function carregarVendas() {
    fetch('vendas.php?buscar=todos', {
        method: 'GET',
    })
    .then((response) => response.json())
    .then(function(responseBody) {
        if (responseBody.totalRegistros > 0) {
            var exemploView = new VendasView();

            exemploView.montarVendas(responseBody.dados);
        }
    })
    .catch(function(err) {
		...
		if (selectAll.checked) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				if (selectBox.checked) {
					toDelete.push(selectBox.id.replace("venda_", ""));
				}
			}
...
class VendasView {
	montarVendas(dados) {
		...
		dados.forEach(element => {
			...
			bodyTD = document.createElement('td');
			bodyTD.innerHTML = element.dataVenda;
			bodyTR.appendChild(bodyTD);

            bodyTD = document.createElement('td');
			bodyTD.innerHTML = element.totalVenda;
			bodyTR.appendChild(bodyTD);
			...
			bodyTD.addEventListener('click', function() {
				window.location.replace("incluirVendas.php?id=" + element.id);
			});
			bodyTR.appendChild(bodyTD);
...
function sortArray(dados) {
	...
	if (data !== '') {
		for (var i = dados.length-1; i >= 0; i--) {
			...
			if (!dados[i].dataVenda.startsWith(data)) {
				delete dados[i];
			}
		}
	}
